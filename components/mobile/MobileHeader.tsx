...
export function MobileHeader() {
  ...
  return <><header className={`m-header${scrolled?' m-header-scrolled':''}`}><a href="/" aria-label="LF Business, home"><img src="/images/logo-lf-clean.png" width="56" height="40" alt="LF" /></a><button ref={toggle} onClick={()=>setOpen(true)} aria-label="Apri menu" aria-expanded={open} aria-controls="lf-mobile-navigation"><span /><span /></button></header><dialog ref={dialog} id="lf-mobile-navigation" className="m-menu" onCancel={()=>setOpen(false)}><div className="m-menu-top"><img src="/images/logo-lf-clean.png" width="56" height="40" alt="LF Business" /><button className="m-menu-close" onClick={()=>setOpen(false)} aria-label="Chiudi menu"><span /><span /></button></div><nav aria-label="Navigazione smartphone"><ol>{links.map(([label,href],i)=><li key={href} style={{animationDelay:`${i*35}ms`}}><a href={href} onClick={()=>setOpen(false)} aria-current={pathname.replace(/\/$/,'')===href.replace(/\/$/,'')?'page':undefined}><span>{String(i+1).padStart(2,'0')}</span>{label}<EditorialArrow direction="external" /></a></li>)}</ol></nav><div className="m-menu-bottom"><LanguageSwitcher light /><a href={`mailto:${CONTACT.email}`}>{CONTACT.email}</a><a href={CONTACT.phoneHref}>{CONTACT.phone}</a></div></dialog></>
}
